...
def preprocess_and_extract_file_level_features(data_path, sample_rate=48000):
    ...
    for file_name in os.listdir(data_path):
        if file_name.endswith('.wav'):
            ...
            # Handle both formats: "farmerjack_10.wav" and "speaker_0000_1.wav"
            parts = file_name.split('_')
            if len(parts) >= 2 and parts[0] == 'speaker':
                label = f"{parts[0]}_{parts[1]}"  # Extract "speaker_0000" from "speaker_0000_1.wav"
            else:
                label = parts[0]  # Extract "farmerjack" from "farmerjack_10.wav"
            labels.append(label)
...
try:
    ...
    print("Note: Chi-square test requires sufficient expected frequencies in all cells")
